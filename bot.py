...
MULTI_SYMBOLS = [s.strip().upper() for s in (os.getenv("MULTI_SYMBOLS", "EURUSDT").split(",") if os.getenv("MULTI_SYMBOLS") else ["EURUSDT"])]
...
async def _send_async(text: str):
    if not TG_TOKEN or not TG_CHAT:
        ...
        return
    from telegram import Bot  # ленивый импорт
...
def load_klines_crypto(symbol: str, interval: str, limit: int = 1000):
    _lazy_heavy_imports()
    s = _make_session()
    i_int = normalize_interval(interval)
    ...
    if MEXC_MODE == "contract":
        try:
            ...
            r2 = s.get(url, params={"symbol": sym_c, "interval": i_c, "limit": limit}, timeout=20)
    ...
    try:
        r3 = s.get("https://api.binance.com/api/v3/klines",
...
# ─────────────────────────── ЛОГИКА СИГНАЛА ───────────────────────────
def sawvap_last_signal(df, symbol: str, interval: str, length: int, tol_pips: float):
    _lazy_heavy_imports()
...
def run_worker():
    ...
    for idx, sym in enumerate(MULTI_SYMBOLS):
        ...
        # сдвигаем первый опрос для каждого инструмента на 10s * idx (но не позже POLL_DELAY-1)
        initial_offset = min(10 * idx, max(POLL_DELAY - 1, 1))
...
@app.head("/")
def root_head():
    # быстрый ответ для keep-alive без старта воркера
