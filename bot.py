# -*- coding: utf-8 -*-
# Переключатель режима (Прорыв тренда Дончиана + Возврат в диапазон) → Telegram (только сообщения)
# Источник свечей: MEXC v3 → MEXC v2 → Binance v3 (откат)
...
DIST_SLOW = float(os.getenv("DIST_SLOW", "0.6"))  # Возврат к среднему при слабом тренде
...
def _fmt_signal_text(side: str, meta: dict, bar_ts: pd.Timestamp) -> str:
    ...
    return (
        ...
        f"Дончиан: H={meta['don_hi_prev']:.5f} L={meta['don_lo_prev']:.5f}"
    )
...
def _df_from_k(arr) -> pd.DataFrame:
    ...
    for need in ["t", "o", "h", "l", "c"]:
        if need not in df.columns:
            raise RuntimeError(f"Unexpected klines формат: '{need}' отсутствует")
...
def load_klines(symbol: str, interval: str, limit: int = 1000) -> pd.DataFrame:
    ...
    try:
        ...
        i_v2 = i_map.get(interval.lower(), "Мин15")  # дефолтно 15m
        r2 = SESSION_HTTP.get(
            url_v2, params={"symbol": sym_v2, "interval": i_v2, "limit": limit}, timeout=20
        )
        ...
        try:
            ...
            if side and self._ok_limits(len(df)) and in_session(datetime.now(timezone.utc)):
                if not _already_sent(side, lt):
                    ...
                    print(f"[{self.label}] пропущен дубликат сигнала | {side} @ {lt}")
            else:
                print(f"[{self.label}] нет сигнала | bar {lt}")
...
def run_worker():
    ...
    print(f"Бот запущен. Источник: MEXC Spot klines (с резервным вариантом). TZ: {TZ_NAME}")
